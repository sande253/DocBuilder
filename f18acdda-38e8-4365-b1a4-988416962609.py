...
class BubbleSortVisualization(manim.Scene):
    def construct(self):
        ...
        for i in range(n - 1):
            ...
            new_pass_label_mobject = manim.Text(f"Pass: {i + 1}", font_size=30).move_to(pass_label.get_center())
            # Animate the transformation of the old label to the new one
            self.play(manim.Transform(pass_label, new_pass_label_mobject))
            pass_label = new_pass_label_mobject # Update reference for subsequent positioning
            self.wait(0.5)
