from manim import *
import random
from manim.mobject.biology import DNA_DoubleHelix # Import DNA_DoubleHelix
...
class ChromosomeScene(Scene):
    def construct(self):
        ...
        # Create the cell circle and its centered label, grouped into a VGroup
        cell_circle = Circle(radius=3, color=WHITE, fill_opacity=0.1)
        cell_text = Text("Cell").move_to(cell_circle.get_center()) # Centered text within the circle
        cell = VGroup(cell_circle, cell_text)

        # Create the nucleus circle and its centered label, grouped into a VGroup
        nucleus_circle = Circle(radius=1.5, color=BLUE_E, fill_opacity=0.3)
        nucleus_text = Text("Nucleus").move_to(nucleus_circle.get_center()) # Centered text within the circle
        nucleus = VGroup(nucleus_circle, nucleus_text)
        nucleus.move_to(cell_circle.get_center()) # Position the nucleus VGroup at the cell's center

        # Animate the creation of the cell and nucleus VGroups
        self.play(Create(cell))
        self.play(Create(nucleus))
        self.wait(1)

        # Zoom into the nucleus by scaling it up and fading out the cell
        # Animate the entire VGroup for cell and nucleus
        self.play(
            cell.animate.scale(0.1).fade(1), # Shrink and fade the entire cell VGroup
            nucleus.animate.scale(2).center() # Scale up and center the nucleus VGroup
        )
        self.wait(0.5)

        # Update nucleus text after scaling. nucleus_text is nucleus[1].
        # Create a new Text Mobject with desired properties and use .become() to update.
        new_nucleus_text_mobject = Text("Nucleus", font_size=36) # Explicitly set font size to counteract previous scaling
        new_nucleus_text_mobject.move_to(nucleus_circle.get_center()) # Center it in the scaled nucleus_circle
        self.play(nucleus_text.animate.become(new_nucleus_text_mobject))
        self.wait(1)
        ...
        # Fade out the nucleus VGroup and introduce the chromosomes
        self.play(
            FadeOut(nucleus), # Fade out the entire nucleus VGroup
            LaggedStart(*[Create(chrom) for chrom in chromosomes], lag_ratio=0.2) # Staggered creation
        )
        self.wait(1)

        # Highlight one chromosome and label it (label is external)
        target_chrom = chromosomes[2]
        original_target_chrom_color = target_chrom.get_color() # Store original color before changing
        ...
        self.wait(1.5)
        self.play(
            target_chrom.animate.scale(1/1.2).set_color(original_target_chrom_color), # Revert to original state using stored color
            FadeOut(chrom_label)
        )
        self.wait(0.5)
        ...
        self.play(
            ...
            single_chrom.animate.scale(0.1).fade(1), # Shrink and fade the chromosome VGroup
            Create(dna_helix),
            Write(dna_label)
        )
        self.wait(2)
        ...
        # Update the chromosome label after its transformation using .become()
        new_chrom_label_text = Text("Chromosome", font_size=30).next_to(single_chrom, UP, buff=0.5)
        self.play(chrom_label_dna.animate.become(new_chrom_label_text))
        self.wait(1)
